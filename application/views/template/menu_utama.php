<div class="wrapper row2">
  <nav id="mainav" class="hoc clear"> 
    <!-- ################################################################################################ -->
    <ul class="clear">
      <li class="active"><a href="index.html">Home</a></li>
      <li><a class="drop" href="#">Profil</a>
        <ul>
          <li><a href="pages/gallery.html">Sambutan Kepala Sekolah</a></li>
          <li><a href="pages/full-width.html">Visi Misi</a></li>
          <li><a href="pages/sidebar-left.html">Sejarah Singkat</a></li>
          <li><a href="pages/basic-grid.html">Struktur Organisasi</a></li>
        </ul>
      </li>
      <li><a class="drop" href="#">Fasilitas</a>
        <ul>
          <li><a href="pages/gallery.html">Gedung</a></li>
          <li><a href="pages/full-width.html">Lapangan</a></li>
          <li><a href="pages/sidebar-left.html">Lab</a></li>
          <li><a href="pages/basic-grid.html">Kantin</a></li>
        </ul>
      </li>
      <li><a href="#">Guru</a></li>
      <li><a href="#">Siswa</a></li>
      <li><a href="#">Kontak Kami</a></li>
      <li><a href="#">Long Link Text</a></li>
    </ul>
    <!-- ################################################################################################ -->
  </nav>
</div>